...
// Aquí pones tu URL de Render directamente
const RENDER_API = "https://backend-pwa-ofj5.onrender.com";
...
const Admin = () => {
  ...
  useEffect(() => {
    const fetchUsuarios = async () => {
      try {
        const res = await axios.get(`${RENDER_API}/api/usuarios`);
        setUsuarios(res.data);
        await saveUsuarios(res.data); // Guardar en IndexedDB para offline
      } catch (err) {
        console.error("Error al cargar usuarios:", err);
        const usuariosOffline = await getUsuarios();
        if (usuariosOffline.length > 0) setUsuarios(usuariosOffline);
      }
    };
    fetchUsuarios();
  }, []);
  ...
  const handleEnviarNotificacion = async (e) => {
    ...
    try {
      const res = await axios.post(`${RENDER_API}/api/usuarios/enviar-notificacion`, {
        ...
        <div style={{ marginTop: "30px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {notificaciones.map((n, index) => (
              <div key={index} style={{ padding: "15px", border: "1px solid #ddd", borderRadius: "4px", backgroundColor: "#f9f9f9" }}>
